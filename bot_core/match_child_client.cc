                                                         ...
                                                         Handler handler)
{
    ...
    // Register BEFORE writing: the child can execute the request and the read thread can
    // dispatch its reply/result before this thread runs again. An unregistered ipc_id would
    // drop the reply (user never sees it) and leak the future until match teardown, where
    // PendingRequests::~PendingRequests answers it with a spurious "游戏已经结束".
