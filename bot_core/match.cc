...
ErrCode Match::GameStart(const UserID uid, MsgSenderBase& reply)
{
    ...
    // Build the player list outside the lock: GetUserName/GetUserAvatar go through platform
    // callbacks (avatar download, fallback rendering) that may block for a long time, and
    // holding sync_ across them froze the match and every command iterating matches.
