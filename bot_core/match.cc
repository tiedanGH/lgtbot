                       ...
                       MsgSender& reply)
{
    ...
    uint32_t multiple = 0;
    // Use TryMatchOption (not SetDefaultOption) so the change stays local to
    // this match — SetDefaultOption mutates the config_runner's shared
    // default_options_ and applied_options_log_, which then leaks into every
    // subsequent match. Global defaults are only changed via %配置.
