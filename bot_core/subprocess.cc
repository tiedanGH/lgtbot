...
// consumed_out receives the bytes actually read, so callers can tell a clean EOF
// (0 bytes at a frame boundary, e.g. the child exited normally) from a mid-frame break.
...
bool Subprocess::Read(std::string& payload_out)
{
    ...
    if (!ReadAll(process_.get(), hdr, sizeof(hdr), consumed)) {
        if (consumed == 0) {
            // Clean EOF at a frame boundary: the child exited normally (e.g. teardown closed
            // its stdin); this is the expected end of stream, not an error.
...
void Subprocess::Close() noexcept
{
    ...
    // Give the child a moment to exit on its own after stdin EOF before escalating: a clean
    // exit closes its stdout at a frame boundary, so the parent read loop sees a silent EOF
    // instead of logging a stream error for every normal teardown.
