...
// markdown2image (Qt WebEngine) can wedge. A render is often performed while holding a
// match's state lock, so an unbounded popen()/pclose() here used to freeze the match and
// every global command that iterates matches. Bound the whole render instead.
...
inline int MarkdownToImage(const std::string& markdown, const std::string& path, const uint32_t width)
{
    ...
    // CLOEXEC on both ends so concurrently spawned processes cannot inherit the write end
    // and keep the renderer's stdin open forever.
