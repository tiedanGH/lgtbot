...
class Board
{
  public:
    ...
    string GetPlayerInfo(const PlayerID pid) const
    {
        html::Table playerTable(1, 6);
        playerTable.SetTableStyle("align=\"center\" cellpadding=\"0\" cellspacing=\"0\"");
    ...
    {
        const vector<UnitMaps::Map>& maps = test_mode == 0 ? unitMaps.maps : (test_mode == 1 ? unitMaps.all_maps : unitMaps.rotation_maps);
        const vector<UnitMaps::Map>& exits = test_mode == 0 ? unitMaps.exits : (test_mode == 1 ? unitMaps.all_exits : unitMaps.rotation_exits);
        const vector<UnitMaps::Map>& special_maps = unitMaps.special_maps;
        const string title = 
            (test_mode == 0) ? "" 
            : (test_mode == 1) ? 
                (HTML_SIZE_FONT_HEADER(6) "<b>《漫漫长夜》狂野模式全部区块</b>" HTML_FONT_TAIL) 
        ...
        if (has_special) {
            ...
            blocks.Get(row++, 0).SetStyle("style=\"padding: 20px 25px 5px 25px\"")
                .SetContent(HTML_SIZE_FONT_HEADER(6) "<b>特殊区块列表<br>" HTML_FONT_TAIL HTML_SIZE_FONT_HEADER(5) "（多传送门默认对角线传送）</b>");
        ...
        legend.Get(3, 1).SetContent("【传送门入口】玩家进入时会发出其他人听见的啪啪声。（出生不算）<br>进入后，再任意2次移动后就会传送至同区块另1个传送门。<br>进入后，玩家视作进入亚空间，上述2次移动都在亚空间内。");
        legend.Get(4, 0).SetContent(generateSVG(GetBgStyle(GridType::ONEWAYPORTAL, true)));
        legend.Get(4, 1).SetContent("【传送门出口】玩家进入时会发出其他人听见的啪啪声。（出生不算）<br>传送门的单向出口，进入时不会触发传送（必须从入口进入才会传送至此处）<br>玩家在进入同一区块的传送门入口时，传送门会转换方向，入口和出口交换位置");
    ...
    // 地区声响（0无 1沙沙 2啪啪）
    int GetSound(const Grid& grid, const bool water_mode) const
    {
        switch(grid.Type()) {
            case GridType::GRASS:   return 1;
            case GridType::WATER:   return 2;
            case GridType::ONEWAYPORTAL:
            case GridType::PORTAL:  return 2;
            case GridType::TRAP:    return grid.TrapStatus() ? 0 : (water_mode ? 2 : 1);
            default: return 0;
        }
    }
    ...
    void TeleportPlayer(const PlayerID pid)
    {
        ...
        for (const auto& pos : candidates) {
            if (!forbidden[pos.first][pos.second])
                finalCandidates.push_back(pos);
        }
    ...
    string MapGenerate(const vector<string>& map_str) const
    {
        ...
        for (int i = 0; i < unitMaps.origin_pos.size(); i++) {
            string map_id;
            bool is_exit = false;
            if (!map_str[i].empty() && (map_str[i][0] == 'E' || map_str[i][0] == 'e')) {
                is_exit = true;
                map_id = map_str[i].substr(1);
            } else {
                map_id = map_str[i];
            }
            if (is_exit) {
        ...
        for (int x = 0; x < size; x++) {
            for (int y = 0; y < size; y++) {
                if (grid_map[x][y].Type() == GridType::PORTAL && grid_map[x][y].IsFullyEnclosed()) {
                    ...
                    grid_map[x + pRelPos.first][y + pRelPos.second].SetType(GridType::WATER).SetPortal(0, 0);
                }
            }
        }
    }
