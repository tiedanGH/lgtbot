...
class MainStage : public MainGameStage<RoundStage>
{
  ...
  private:
    ...
    {
        bool game_end = (Alive_() <= 1 && Global().PlayerNum() > 1) || ((Alive_() <= 0 || board.exit_num == 0) && Global().PlayerNum() == 1);
...
class RoundStage : public SubGameStage<>
{
   public:
    RoundStage(MainStage& main_stage, const uint64_t round)
            : StageFsm(main_stage, "第 " + std::to_string(round) + " 回合" ,
                MakeStageCommand(*this, "选择方向，在迷宫中探路", &RoundStage::MakeMove_, AlterChecker<Direct>({{"上", Direct::UP}, {"下", Direct::DOWN}, {"左", Direct::LEFT}, {"右", Direct::RIGHT}})),
    ...
    virtual void OnStageBegin() override
    {
        ...
        Global().Boardcast() << "请 " << At(currentPlayer) << " 在公屏选择方向移动（第一次行动前有 " << GAME_OPTION(思考时限) << " 秒思考时间，行动开始后总时限为 " << GAME_OPTION(行动时限) << " 秒）";
        Global().StartTimer(GAME_OPTION(思考时限));
        if (Main().round_ == 1) {
            if (GAME_OPTION(BOSS)) SendSoundMessage(Main().board.boss.x, Main().board.boss.y, 3, true);
   ...
   private:
    AtomReqErrCode MakeMove_(const PlayerID pid, const bool is_public, MsgSenderBase& reply, Direct direction)
    {
        ...
        if (!success) {
            if (step == 1) {
                reply() << UnitMaps::firststep_wall_hints[rand() % UnitMaps::firststep_wall_hints.size()] << "\n移动时碰撞【墙壁】，本回合结束！**请留意机器人私信发送的四周墙壁信息**";
            } else {
                reply() << UnitMaps::wall_hints[rand() % UnitMaps::wall_hints.size()] << "\n移动时碰撞【墙壁】，本回合结束！请留意机器人私信发送的四周墙壁信息";
            }
            return StageErrCode::READY;
        }

        auto sender = reply();
        Grid& grid = Main().board.PlayerLocationGrid(pid);
        if (player.subspace <= 0) {
            ...
            if (grid.Type() == GridType::EXIT) {
                ...
                player.score.exit_score += Score::exit_order[Main().board.exited++];     // 逃生分

                if (GAME_OPTION(大乱斗)) {
                    player.move_record += "【传送】";
                    Main().board.TeleportPlayer(pid);  // 大乱斗随机传送
                    sender << UnitMaps::exit_hints[rand() % UnitMaps::exit_hints.size()] << "\n\n您已抵达【逃生舱】！此逃生舱已失效，" << At(pid) << " 被随机传送至地图其他地方！";
                } else {
                    player.out = 2;
            ...
            if (player.subspace == 0) {
                ...
                Main().board.player_map[player.x][player.y].push_back(pid);
            ...
            if (grid.Type() == GridType::TRAP) {
                ...
                if (grid.TrapStatus()) {
                    ...
                    sender << UnitMaps::trap_hints[rand() % UnitMaps::trap_hints.size()] << "\n\n移动触发【陷阱】，本回合被强制停止行动！";
                    return StageErrCode::READY;
                }
            }
            // 热源
            string step_info, heat_message;
            if (Main().board.HeatNotice(pid)) {
                step_info = "[热浪(第" + to_string(step) +  "步)]";
                heat_message = string(UnitMaps::heat_wave_hints[rand() % UnitMaps::heat_wave_hints.size()]) + "\n移动进入【热浪范围】，当前位置附近存在热源";
            }
            if (grid.Type() == GridType::HEAT) {
                if (Main().board.player_map[player.x][player.y].size() > 1) {
                    step_info = "[热源(第" + to_string(step) +  "步)]";
                    heat_message = "已经有人在这里了，炽热的环境让你难以辨认对方，但是他的存在使这里的温度比想象中缓和了一些。\n移动进入【热源】！本次不记录进入热源总次数";
                } else if (player.heated) {
                    player.move_record += "(热源出局)";
                    player.out = 1;
                    player.score.quit_score = -150;
                    if (Global().PlayerNum() > 1) Global().Eliminate(pid);
                    sender << UnitMaps::heat_end_hints[rand() % UnitMaps::heat_end_hints.size()] << "\n\n本局游戏第二次进入【热源】，高温难耐，您失去了行动能力！";
                    if (Main().Alive_() > 1) {
                        Main().board.UpdatePlayerTarget(GAME_OPTION(捉捕目标));   // 捕捉顺位变更
                        sender << "\n\n剩余玩家捕捉目标顺位发生变更！\n";
                        sender << Markdown(Main().board.GetPlayerTable(Main().round_));
                    ...
                    player.heated = true;
                    step_info = "[热源(第" + to_string(step) +  "步)]";
                    heat_message = string(UnitMaps::heat_core_hints[rand() % UnitMaps::heat_core_hints.size()]) + "\n移动进入【热源】！请注意，在下一次进入热源时，将直接出局并-150分";
                }
            }
            if (heat_message != "") {
            ...
            // 声响
            int sound = Main().board.GetSound(grid, GAME_OPTION(特殊事件) == 3);
            if (sound == 1) {
                if (hide) {
                    sender << "移动进入【树丛】（隐匿中，不会向其他人发出声响）\n\n";
                } else {
                    player.move_record += "[沙沙]";
                    sender << UnitMaps::grass_hints[rand() % UnitMaps::grass_hints.size()] << "\n移动进入【树丛】，请其他玩家留意私信声响信息！\n\n";
                ...
                if (hide) {
                    ...
                    sender << UnitMaps::papa_hints[rand() % UnitMaps::papa_hints.size()] << "\n移动发出【啪啪声】，请其他玩家留意私信声响信息！\n\n";
                    SendSoundMessage(player.x, player.y, sound, false);
                }
            }
            if (!GAME_OPTION(点杀)) {
        ...
        // 单步隐匿：消除隐匿状态
        if (GAME_OPTION(隐匿) == 2) { hide = false; }
        return StageErrCode::OK;
    }
    ...
    bool PlayerCatch(Player& player, MsgSenderBase::MsgSenderGuard& sender)
    {
        ...
        if (find(list.begin(), list.end(), t) != list.end() && !hide) {
            active_stop = false;
            sender << UnitMaps::catch_hints[rand() % UnitMaps::catch_hints.size()] << "\n\n";
            sender << At(t) << " 被捕捉！";
            ...
            if (Main().Alive_() == 1) {
                // 无逃生舱最后生还判定
                if (Main().board.exit_num == 0) Main().withoutE_win_ = true;
                // 有逃生舱但死斗取胜判定
                if (Main().board.exit_num > 0) Main().withE_win_ = true;
            }
            return true;
        }
        return false;
    ...
    // 私信其他玩家发送声响信息
    void SendSoundMessage(const int fromX, const int fromY, const int sound, const bool to_all)
    {
        for (PlayerID pid = 0; pid < Global().PlayerNum(); ++pid) {
            if ((pid != currentPlayer || to_all) && Main().board.players[pid].out == 0) {
                ...
                if (direction == "") {
                    if (Main().board.players[currentPlayer].target != pid || GAME_OPTION(点杀)) {
                        switch (sound) {
                            case 1: sound_message = step_info + "\n" + string(UnitMaps::grass_sound_hints[rand() % UnitMaps::grass_sound_hints.size()]); break;
                            case 2: sound_message = step_info + "\n" + string(UnitMaps::papa_sound_hints[rand() % UnitMaps::papa_sound_hints.size()]); break;
                            default: sound_message = "[错误] 未知声音类型：相同格子的未知声音";
                        }
                    }
                } else {
                    switch (sound) {
                        case 1: sound_message = step_info + "你听见了来自【" + direction + "方】的沙沙声！"; break;
                        case 2: sound_message = step_info + "你听见了来自【" + direction + "方】的啪啪声！"; break;
                        case 3: sound_message = "[BOSS] 你听见了来自【" + direction + "方】的巨大响声！"; break;
    ...
    CheckoutErrCode HandleStageOver()
    {
        ...
        Global().Boardcast() << "[" << currentPlayer << "号]玩家本回合的完整行动轨迹：\n" << player.move_record;
        // 记录历史行动轨迹
        ...
        if ((Main().Alive_() == 1 && Global().PlayerNum() > 1) || (Main().Alive_() == 0 && Global().PlayerNum() == 1)) {
            if (Main().withoutE_win_) {     // 无逃生舱最后生还胜利
                Global().Boardcast() << At(currentPlayer) << "\n" << UnitMaps::withoutE_win_hints[rand() % UnitMaps::withoutE_win_hints.size()];
            }
            if (Main().withE_win_) {        // 有逃生舱但死斗取胜
                Global().Boardcast() << At(currentPlayer) << "\n" << UnitMaps::withE_win_hints[rand() % UnitMaps::withE_win_hints.size()];
            }
            return StageErrCode::CHECKOUT;
        }
