...
GAME_TEST(2, one_do_nothing)
{
    ...
    ASSERT_PUB_MSG(OK, 0, "副数 5");
    ASSERT_TRUE(StartGame());
    ASSERT_PUB_MSG(OK, 0, "C4"); // 红4, adjacent to stone C3 (4-direction)
    ASSERT_TIMEOUT(CHECKOUT); // player_1 hooks
    for (uint32_t i = 0; i < 39; ++i) {
        ASSERT_TIMEOUT(CHECKOUT); // both hook
    }
    ASSERT_SCORE(0, 0);
}
...
GAME_TEST(2, restore_hook)
{
    ...
    ASSERT_PUB_MSG(OK, 1, "C4"); // restore hook player_1, place on their own board
    ASSERT_PUB_MSG(CHECKOUT, 0, "C2"); // 灰5, adjacent to stone C3
}
