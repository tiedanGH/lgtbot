...
// formal questions
constexpr static uint32_t k_question_num = 58;
// with test questions
constexpr static uint32_t all_question_num = 107;
...
char* find_question(string keywords)
{
...
void MainStage::FirstStageFsm(SubStageFsmSetter setter)
{
    ...
    if(GAME_OPTION(测试模式)) {
        Global().Boardcast() << "[警告] 正在进行新题测试！请勿开启倍率，如出现异常，请联系管理员中断游戏。";
    }
